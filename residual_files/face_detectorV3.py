    ...
    def face_detector(self):
        def draw_boundary(img, classifier, scaleFactor, minNeighbors, color, text, clf):
            ...
            for (x, y, w, h) in features:
                ...
                if confidence > 77:
                    cv2.putText(img, f"Roll:{roll_no}", (x, y - 55), cv2.FONT_HERSHEY_COMPLEX, 0.8, (255, 255, 255), 3)
                    cv2.putText(img, f"Department:{dept}", (x, y - 30), cv2.FONT_HERSHEY_COMPLEX, 0.8, (255, 255, 255), 3)
                    cv2.putText(img, f"Year:{year}", (x, y - 5), cv2.FONT_HERSHEY_COMPLEX, 0.8, (255, 255, 255), 3)
                else:
